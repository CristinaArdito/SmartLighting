...
public class PannelloControlloSimulazione extends JFrame {
	...
	public PannelloControlloSimulazione(List<Stanza> stanze, Configurazione config, Interfaccia frame) {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					if (listaSensoriAttivi.get(listaSensori.getSelectedIndex()) == true) {
						...
						modelloSensori.setElementAt("<html><div style='color:red'>"
								+ config.getDispositivi().get(listaSensori.getSelectedIndex()).getId() + " - "
								+ config.getDispositivi().get(listaSensori.getSelectedIndex()).getTipo() + " - ID: "
								+ config.getDispositivi().get(listaSensori.getSelectedIndex()).getId()
								+ "</div></html>", listaSensori.getSelectedIndex());
					} else {
						...
						modelloSensori.setElementAt("<html><div style='color:green'>"
								+ config.getDispositivi().get(listaSensori.getSelectedIndex()).getId() + " - "
								+ config.getDispositivi().get(listaSensori.getSelectedIndex()).getTipo() + " - ID: "
								+ config.getDispositivi().get(listaSensori.getSelectedIndex()).getId()
								+ "</div></html>", listaSensori.getSelectedIndex());
		...
		JButton btnRiavviaSistema = new JButton("Riavvia Sistema");
		btnRiavviaSistema.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				interfaccia.setVisible(true);
				sistema = new Sistema(stanze, config);
				sistema.Control();
			}
		});

		btnRiavviaSistema.setBounds(10, 639, 230, 32);
		pannelloSinistro.add(btnRiavviaSistema);
	...
	public void riempiStanze(JPanel pannelloDestro, List<Stanza> stanze) {
		...
		for (Stanza stanza : stanze) {
			stanzaDiAppoggio = new EspositoreStanze(stanza, stanza.getNome(), stanza.getDispositivi(), stanza.getLuci(),
					stanza.getSensore());
			...
			if (y + 280 > pannelloDestro.getHeight()) {
				pannelloDestro
						.setPreferredSize(new Dimension(pannelloDestro.getWidth(), pannelloDestro.getHeight() + 300));
			}
		}
	...
	private class EspositoreStanze {
		...
		public EspositoreStanze(Stanza stanza, String nome, List<Dispositivo> dispositivi, List<Luce> luci,
			...
			for (Luce luce : luci) {
				...
				case 1:
					modello.addElement(
							"<html>Luce n: " + luce.getId() + " - Stato: <font color=green>accesa</font></html>");
					break;
				case -1:
					modello.addElement(
							"<html>Luce n: " + luce.getId() + " - Stato: <font color=red>spenta</font></html>");
					break;
				}
			}
